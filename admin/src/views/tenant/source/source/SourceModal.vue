...
import {
  ...
  editSourceApi,
  connectionSourceApi,
} from '@service/tenant/source/source'
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    async function connection() {
      const values = await validate()
      await connectionSourceApi(values).then(() =>
        createMessage.success('数据源连接成功！'),
      )
    }

    return { registerModal, registerForm, getTitle, handleSubmit, connection }
  },
})
</script>
