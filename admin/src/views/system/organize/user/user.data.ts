...
import { dictConversion, isEmpty, isEqual, pull } from '@admin/utils'
import { DicSortEnum, DicStatusEnum } from '@enums/basic'
import { UserIM } from '@type/system'
import { DefaultPassword, OrganizeTypeEnum, SexEnum } from '@enums/system'
import { organizeOptionApi } from '@service/system/organize/organize'
    ...
    componentProps: ({ formModel }) => {
      return {
        ...
        onSelect: (value: string | number, node: any) => {
          if (
            formModel !== undefined &&
            isEqual(node?.type, OrganizeTypeEnum.DEPT)
          ) {
