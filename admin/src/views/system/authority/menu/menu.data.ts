...
import { dictConversion, isEmpty } from '@admin/utils'
import {
  DicSortEnum,
  DicStatusEnum,
  DicYesNoEnum,
...
import { MenuIM } from '@type/system'
import { optionModuleApi } from '@service/system/authority/module'
import {
  getMenuRouteListApi,
  getMenuRouteListExNodesApi,
} from '@service/system/authority/menu'
import {
  COMMON_MENU,
  COMMON_MODULE,
  FrameTypeEnum,
...
import { ColorEnum } from '@admin/tokens'
...
export const columns: BasicColumn[] = [
  ...
  },
  {
    ...
    customRender: ({ record }) => {
      ...
      return !data.perms
        ? data.perms
        : h(Tag, { color: ColorEnum.ORANGE }, () => data.perms)
    },
  },
  {
    title: '状态',
...
export const formSchema: FormSchema[] = [
        ...
        onChange: async (e: any) => {
          const treeData =
            e === undefined
              ...
              : await getMenuRouteListExNodesApi(
                  formModel.id,
                  e,
                  MenuTypeEnum.DETAILS,
                )
  ...
  },
  {
    ...
    required: ({ values }) =>
      isDetails(values.menuType) ||
      isNormalMenu(values.menuType, values.frameType),
    ifShow: ({ values }) =>
      isDetails(values.menuType) ||
      isNormalMenu(values.menuType, values.frameType),
    colProps: { span: 12 },
  },
  {
    ...
    ifShow: ({ values }) =>
      isDetails(values.menuType) || isMenu(values.menuType),
    colProps: { span: 12 },
  },
  {
    label: '权限标识',
